...
class LinkedBST(AbstractCollection):
    ...
    def height(self):
        ...
        def _height1(top):  # time is linear in size of subtree
            """Return the height of the subtree rooted at Position p."""
            if top is None:
                return -1
            return max(_height1(top.left), _height1(top.right)) + 1
    ...
    def isBalanced(self):
        ...
        def recursive_balanced(node):
            if node.left is None and node.right is None:
                ...
                return recursive_balanced(node.left) and recursive_balanced(node.right)

        return recursive_balanced(self._root)
    ...
    def rebalance(self):
        ...
        def inject(res_list, cur_tree):
            if len(res_list):
                middle = len(res_list) // 2
                cur_tree.add(res_list[middle])
                inject(res_list[middle + 1:], cur_tree)
                inject(res_list[:middle], cur_tree)
    ...
    def predecessor(self, item):
        ...
        recurse(self._root)
        return stack.pop() if not stack.isEmpty() else None
